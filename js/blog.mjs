...
let postContent =  document.querySelectorAll('.postContentContainerCommonClass');

for (let index = 4; index < postContent.length; index++) {
  postContent[index].style.display = 'none';
...
nextButtonBlog.addEventListener('click',()=>{
      postContent.forEach((element,index) => {
      if (index > 4) {
